...
import { getBikeData } from "../Redux/AppReducer/action";

const SingleProductBike = () => {
  const dispatch = useDispatch();
  const { id } = useParams();

  const [currentBike, setCurrentBike] = useState({});
  const bikes = useSelector((state) => state.AppReducer.bikeData);
  useEffect(() => {
    if (bikes.length === 0) {
      dispatch(getBikeData());
    }
  }, [dispatch, bikes.length]);

  useEffect(() => {
    if (id) {
      const singlebike = bikes.find((item) => item._id === id);
            ...
            <Box textAlign="left">
              ...
              <Box display="flex" marginTop="10px">
                <Box fontWeight={400} fontSize="14px" lineHeight="40px">
                                  <Text>A good condition { currentBike.model} is for sale.</Text>
                </Box>
              </Box>
            </Box>
          </Box>
...
export default SingleProductBike;
